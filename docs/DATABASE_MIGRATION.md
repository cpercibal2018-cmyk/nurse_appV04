...
The JSON column `credential_templates.field_defs_legacy` was **kept** (unread) as a rollback reference after this migration, and **dropped** on 2026-09-24 during go-live preparation (migration `20260926090000_drop_field_defs_legacy`, see §8).
...
| `field_defs_legacy` and the `pre-db-first` dumps | Owner decision (2026-09-24): retire both during go-live preparation — steps in [DEPLOYMENT.md §4](DEPLOYMENT.md#go-live-preparation-before-creating-the-production-database). **Done 2026-09-24 at the owner's request:** fresh backup `nurseapp_v04_pre-go-live_2026-09-24.dump` (new rollback point, verified with `pg_restore -l`); column dropped by `20260926090000_drop_field_defs_legacy` on the development and test databases, no drift, 16 types / 68 fields intact, 322 / 322 tests; Git tag `pre-db-first` kept (local and GitHub). The owner deleted the two `pre-db-first` dump files the same day. **Closed.** |
