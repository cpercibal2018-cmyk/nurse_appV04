...
| Logging | JSON logs with `X-Request-Id`; no PII in logs; the `request_audit_log` table (D-52) | Spec §9.2 |
...
| :--- | :--- | :--- |
| Auth (+ password change, rate limit, own login/session history — D-22), accounts, scoped role assignments, four-eyes, PAM | SSO (IdP undecided); MFA built later (D-51) | Password reset; username login (D-21) |
...
| Eligibility engine + state + grace + waivers + transitions | Consistency auditor (built), shadow mode (built, D-60) | Acuity, skill mix, mandatory posts |
...
| Idempotency on onboarding/bulk/publish; worker leases | FHIR (built, D-61), exit package (built, D-62), migration bridge | |
...
| D-44 (2026-09-24) | **Onboarding stays in the application; no `fn_onboard_employee_with_contract`** — spec §10.7 item not applied | The employee and first contract are created in one application transaction (contract-first, tested). The runtime role keeps `INSERT` on `employees`; a database function would duplicate the same rules. Closes the open item in [ops/db](../ops/db/README.md#adaptations-from-the-spec-documented-not-silently-chosen) |
| D-45 (2026-09-24) | **The audit reader also reads `break_glass_events` and `privileged_sessions`**, read-only | Auditors can review every emergency access and privilege elevation. Security events and user ids only, no clinical or salary data. Granted in `ops/db/02_grants.sql`, tested in CI |
| D-46 (2026-09-24) | **Sign-in attempt limits are counted in the database** (`login_throttle`) | Replaces the in-memory counters (correct only for one API process, reset on restart). One atomic upsert per failure on the database clock; keys stored as SHA-256 hashes (no e-mail or client address); expired rows swept. Limits and window unchanged (D-23) |
| D-47 (2026-09-24) | **E-mail through the hospital SMTP relay** | Reminders, the break-glass alert (spec §3.6), password reset and invitations. Hospital IT supplies host, port and a service account. **Built 2026-09-24:** notifications are an outbox sent by the e-mail dispatcher (spec §7.2: 60 s, 10-minute lease, 8 attempts, at-least-once, STARTTLS required); break-glass e-mail to `BREAK_GLASS_ALERT_EMAILS` via `email_outbox`. Password reset and invitations are separate flows, not built ([DEPLOYMENT.md §2.2](DEPLOYMENT.md#22-e-mail-hospital-smtp-relay)) |
| D-48 (2026-09-24) | **SMS through the hospital SMS gateway** | Break-glass alert to the CEO and IT Director (spec §3.6); optional reminders. Hospital IT supplies the gateway API details. **Built 2026-09-25 behind a mock gateway (D-59)**; the live driver waits for the CST Sender ID |
| D-49 (2026-09-24) | **Production hosting: Google Cloud, Dammam (`me-central-2`)** | Container images and a deployment pipeline for Google Cloud. The hospital SMTP relay and SMS gateway are on the hospital network, so the Google Cloud project needs private connectivity to it (VPN or Interconnect) — a hospital IT prerequisite. **Built 2026-09-24:** Compute Engine VM with Docker Compose (API, worker, nginx, ClamAV), PostgreSQL 15 self-managed on a second VM, GitHub Actions build → approval → IAP deploy; regional external HTTPS load balancer so TLS ends in the Kingdom ([ops/gcp/README.md](../ops/gcp/README.md)). Tested locally with containers; not provisioned. **Superseded as the production layout by D-57 (single VPS)**; kept as an alternative |
| D-50 (2026-09-24) | **Password reset by role** | Staff accounts (no active HR / Supervisor / System Admin assignment): self-service "Forgot password?" e-mails a single-use link valid 30 minutes; the request always answers the same, so it reveals nothing. HR, supervisor and admin accounts: only HR or a System Admin (in scope, never for their own account) sends the link, valid 24 hours, after confirming who is asking. Break-glass: never in the app. Completing a reset signs out every session and clears the sign-in lockout |
| D-51 (2026-09-24) | **MFA for privileged accounts, without SSO** (spec §3.5) | Authenticator app (TOTP, RFC 6238: SHA-1, 6 digits, 30 s, ±1 step; each step accepted once) plus 10 single-use recovery codes. **Required** for System Admin, HR Admin and Supervisor (`MFA_REQUIRED_ROLES`; production must include SYSTEM_ADMIN and HR_ADMIN): set up at the next sign-in before any session is issued. **Optional** for staff. **Never** for break-glass (spec §3.6 — it must work from the sealed envelopes; the siren is its control). Seeds encrypted with AES-256-GCM (`MFA_ENCRYPTION_KEY`); codes and challenges stored as SHA-256. Wrong codes: 5 per challenge, and the sign-in limits per account and client. Lost phone: a recovery code, or HR / a System Admin (in scope, never their own) resets it — sessions revoked, audited HIGH, the person notified. SSO stays deferred (identity provider not chosen) |
| D-52 (2026-09-24) | **Request-level forensic log** (spec §9.2) | `request_audit_log`: every `/api` request except `GET /health` — request id, time, actor, effective roles, session family, method, path **without** the query string, status, duration, client address, user agent, error code, and the JSON body only as an HMAC-SHA256 (key derived from `JWT_SECRET`) with passwords, tokens, challenges, codes and secrets removed. Written after the response in batches (1 s / 200 rows); a failed write is logged and dropped, never failing a request (spec: fire-and-forget). Append-only (trigger, and no `UPDATE` for the runtime role); the audit reader may read it. **Retention 365 days** — the spec sets none (REQUIREMENT NOT ESTABLISHED); the `request-log-purge` job deletes older rows daily. System Admin viewer: Audit → Requests |
| D-53 (2026-09-25) | **Document vault** (spec §5.3.1) | Stored evidence and contract copies go through a storage-adapter interface (local disk under `STORAGE_DIR` today; private object storage later, once hosting is decided). Every object is **encrypted** (AES-256-GCM, a random data key per object wrapped by `DOCUMENT_ENCRYPTION_KEY`, the object key bound in) and **checked on every read** against the SHA-256 recorded at upload — an altered or missing object is refused (500 `DOCUMENT_INTEGRITY_FAILED` / `DOCUMENT_MISSING`, audited HIGH), never served. **Links:** after the usual authorisation, a single-use link valid **60 s** opens a PDF or image in a browser tab (`GET /files/:token/:fileName`, `Content-Security-Policy: default-src 'none'`); token and name never logged. **Daily `vault-reconcile`:** orphan objects older than a day removed, missing objects and a 50-document integrity sample reported to System Admins, unencrypted objects counted. Earlier plaintext uploads: `npm run vault:encrypt`. The spec's separate quarantine store is not built — D-43 scans before anything is stored |
| D-54 (2026-09-25) | **PDPL field protection** (spec §8.3.1–8.3.4) | V04 keeps identifiers in credential tracking data, not in employee columns, so the protection is per **credential-type field**: a text field may be marked `IQAMA`, `PASSPORT` or `SCFHS_REG` (the hospital baseline marks `iqama_number`, `passport_number`, `scfhs_number`). Such a value is sealed with the **employee's own data key** (AES-256-GCM, employee and field bound in; the key stored only wrapped by `PDPL_FIELD_ENCRYPTION_KEY` in `employee_keys`) — so erasure can destroy one key (crypto-shredding, next step). **Blind index** `pdpl_identifier_index`: HMAC-SHA256 with `PDPL_BLIND_INDEX_PEPPER`, normalised (case, spaces, dashes), exact-match search for HR scope only, the search audited without the value. **Processing register** (§8.3.2): lawful basis, purpose and retention per category, seeded; a category without an active entry cannot be written (409); System Admins change it, audited HIGH. **Log redaction:** ID-shaped numbers masked in every log line. Visibility unchanged: HR and the employee see the values, supervisors never. Earlier values: `npm run pdpl:protect`. The audit trail never held these values, so no audit encryption is needed |
| D-55 (2026-09-25) | **Data-subject rights** (spec §8.3.3) | Table `data_subject_requests` (ACCESS, PORTABILITY, RECTIFICATION, ERASURE; RECEIVED → IN_REVIEW → APPROVED/REJECTED/COMPLETED), one open request per type and employee, never deleted and final once closed (trigger; runtime has no DELETE). The employee requests from **My data**; HR (`pdpl.requests`, scoped) can log one on the employee's behalf and works the queue in **Nursing Administration → Data protection**; nobody decides their own. Answer within **30 days** (PDPL implementing regulation); overdue requests raise `PDPL_REQUESTS_OVERDUE` in System health. **Access / portability:** approval completes the request; the package (JSON: profile, account, contracts, credentials with sensitive values opened, waivers, eligibility, published shifts, attendance, notifications, earlier requests, the processing purposes; files listed, not attached) is built from live data on each download for 30 days, each download audited HIGH. **Rectification:** approve, correct through the usual screens, complete with a note. **Erasure** (`pdpl.erase`, System Admin; four eyes — not whoever logged it, also a CHECK; job number typed to confirm): leftover plaintext sealed, then the employee's key destroyed and their blind-index rows removed; their **identity scans** (files of credential types with a sensitive field) marked erased and deleted from the vault (the daily vault check removes any left behind; an erased file answers 410). Ordinary employment data stays under its lawful basis. Evidence on the request: key destruction time, backups expiry = destruction + `BACKUP_RETENTION_DAYS` + 1 day (the next nightly backup), scans deleted; audited HIGH |
| D-56 (2026-09-25) | **Key rotation and DPO sign-off** (B-18; spec §8.3.2, §8.3.4) | Each of `MFA_ENCRYPTION_KEY`, `DOCUMENT_ENCRYPTION_KEY`, `PDPL_FIELD_ENCRYPTION_KEY` and `PDPL_BLIND_INDEX_PEPPER` accepts a `<NAME>_PREVIOUS` beside it. While both are set, anything sealed under either opens (AES-GCM authentication tells them apart, so ciphertexts carry no label) and new data uses the current key; a search matches digests of both peppers. `npm run keys:rotate` (`--check` to count) re-wraps employee data keys (sealed values unchanged), rebuilds the search index, re-seals MFA seeds and re-wraps each file's data key in place (atomic replace, body and SHA-256 unchanged); audited `KEYS_ROTATED`. A key's id (31-bit HMAC of the key) is stored in `employee_keys.key_version` and `pdpl_identifier_index.key_version`, so System health counts rows under an older key and raises `KEY_ROTATION_PENDING` while any remain or a previous key is configured. Production refuses a previous key without a new one or equal to any other key. **DPO sign-off:** `processing_register_sign_offs` (append-only by trigger) records who signed, their role, a note and the register as reviewed; `pdpl.signoff` (HR, System Admin) with an explicit confirmation; `PDPL_REGISTER_SIGN_OFF_DUE` until the first, 365 days after the last, and after any register change. The key store itself (KMS / HSM) stays with the hosting decision |
| D-57 (2026-09-25) | **Single-VPS production layout** (owner: moving from Google Cloud to a self-managed VPS; spec §10.4, §8.3.6) | [ops/vps](../ops/vps/README.md): Ubuntu 24.04; PostgreSQL 15 **on the host** (so ops/db and the ops/backup WAL/PITR kit apply unchanged), listening only on localhost and the Docker host address, reached by the containers as `db.host`; Caddy (automatic Let's Encrypt, HTTP→HTTPS) in front of **two release colours** (blue/green: api, worker, web each) and a shared ClamAV. `deploy.sh release`: migrations as `nurseapp_migration` → `02_grants.sql` + `verify.sql` → idle colour started and checked (nginx → API → database) → Caddy switched by graceful reload → old worker stopped before the new one starts → old colour stopped and kept for `rollback`; `restart` re-deploys the live release after a settings change. Images are built by GitHub Actions and sent with the scripts over SSH to a deploy key that can run only `nurseapp-release` (bundle checked: `ops/…` and `images.tar.gz` only). `setup-host.sh` (keys-only SSH, ufw 22/80/443, fail2ban, unattended upgrades, Docker, PostgreSQL), `db-init.sh` (database, four roles, random passwords, generated keys, settings files 0600, refuses to overwrite), `setup-backup.sh` (public backup key only on the VPS, WAL archiving, nightly base backup, 15-minute off-site mirror to a versioned bucket). Local proof: 752 requests during release, rollback and restart, 0 failed. **Residency:** the provider's data centre must be in the Kingdom; Hostinger has none (staging with synthetic data only). The Google Cloud layout (D-49) stays available |
| D-58 (2026-09-25) | **Recovery point: at most 5 minutes of changes lost** (B-22; spec §10.6) | Owner decision: `archive_timeout = 300` (PostgreSQL closes and archives a WAL segment at least every 5 minutes; the kit's value is kept). On the single VPS the off-site copy (`nurseapp-offsite.timer`) now also runs every 5 minutes, so a lost server costs at most about 5 minutes of changes plus the copy in flight, not 20; the monitor alerts when the off-site copy is more than 20 minutes old. A smaller bound (1 minute) was offered and not chosen |
| D-59 (2026-09-25) | **SMS through one gateway; mock driver until the CST Sender ID exists** (D-48) — *superseded by D-66 (Telegram)* | A live Saudi SMS gateway needs a CST-registered Sender ID, which needs the hospital's Commercial Registration — not available yet. Every text goes through `SmsGateway.send(phone, message)` (`backend/src/lib/sms.ts`); `SMS_DRIVER=mock` (default) saves it to `mock_sms_outbox` (status `intercepted`, E.164 and length CHECKs, purged after 30 days by `mock-sms-purge`) and sends nothing; `SMS_DRIVER=unifonic` binds the live driver, a stub until the Sender ID exists. `send` never throws (false + log). First caller: the break-glass text to `BREAK_GLASS_ALERT_PHONES`, after the sign-in commits, audited HIGH `BREAK_GLASS_SMS_SENT`/`_FAILED`. Dev Console: Nursing Administration → SMS inbox (`devconsole.sms.read` / `.send`, System Admin, elevated) lists the texts newest first and sends an audited test text for demonstrations |
| D-60 (2026-09-25) | **Shadow mode for eligibility logic** (spec §10.9) | `ENGINES` in `backend/src/modules/eligibility/logic.ts` lists the engine versions a release ships; `eligibility_logic_versions` holds one ACTIVE, at most one SHADOW (partial unique indexes), the rest RETIRED — version 1 is ACTIVE from the migration. At start-up the API and worker register a newly shipped version above the ACTIVE one as SHADOW (replacing an older SHADOW). Every stored evaluation (`refreshEligibility`: events, daily transition, consistency audit) runs the ACTIVE engine and the SHADOW one on the same facts; a different status (or a failure, `ERROR`) is kept once per version, nurse, day and pair of outcomes in `eligibility_shadow_log` (never deleted; a decision is final — trigger). The shadow result is never shown, stored or acted on; previews, pool checks and publication use the ACTIVE engine only (the spec's "every time … calculates" read as every stored calculation). Promotion (spec criteria): 7 days in shadow with no disagreement, or every disagreement APPROVED; a REJECTED one or a failure blocks for good (retire the version). Decisions, promotion and retirement: a system-wide HR Admin with a reason, audited HIGH; after promotion every live nurse is recalculated in the background (the daily transition catches any miss). A rolled-back release that lacks the ACTIVE version falls back to the last promoted version it ships (logged). Health: `ELIGIBILITY_SHADOW_UNDECIDED`, `ELIGIBILITY_LOGIC_STALE`. UI: Nursing Administration → Eligibility logic |
| D-61 (2026-09-25) | **FHIR R4 read API** (spec §14.1) | `backend/src/modules/interop`: GET `/api/v1/fhir/metadata`, `Practitioner/:id`, `PractitionerRole/:id`, `Practitioner?identifier=`, `PractitionerRole?practitioner=`; `application/fhir+json`, route errors as `OperationOutcome`. Mapping typed against the official R4 definitions (`@types/fhir`). Employee → Practitioner (job number, name, work e-mail — not the personal or emergency phone — and verified current licences as `Practitioner.qualification`, SCFHS number opened from its sealed field); unit and position → PractitionerRole (`specialty`, `code`), contract covering today → `period`/`active`. **Deviation:** the spec puts qualifications on PractitionerRole, which has no such element in R4. Callers: a signed-in HR or System Admin (`fhir.read`), limited to their scope; out-of-scope reads as 404, deleted as 410. CI job `fhir-validate` runs the HL7 validator (pinned 6.10.4, `-tx n/a`) on samples from the same code (spec §11.3). Machine-to-machine access: D-63 |
| D-62 (2026-09-25) | **Hospital exit package** (spec §14.3) | A server-side command, never an API: `node dist/cli/exit-package.js --reason … --out <file \| ->` (`npm run exit:package`). One ZIP (yazl, ZIP64 when needed): `organisation.json`, `accounts.json` (no password or MFA secrets), `workforce_master.json/.csv` (deleted employees too), `contract_history.json/.csv`, `contract_events.csv`, `credential_archive.json/.csv` (sensitive values opened), `evidence_archive/EMP_<id>_<TEMPLATE\|CONTRACT>_<YYYYMMDD>[_n].<ext>` (decrypted and integrity-checked; erased and unreadable ones listed, never fatal), `audit_manifest.csv` (the hash chain with each hash's exact inputs; the README gives the formula), `README.txt`, `MANIFEST.sha256`. JSON exact; CSV neutralises spreadsheet formulas. On the VPS `deploy.sh exit-package "<reason>"` streams it from the API container into `gpg` with the backup public key — no plaintext on disk — to `/srv/nurseapp/exit/` (not synced off-site). Audited HIGH `EXIT_PACKAGE_CREATED` |
| D-63 (2026-09-25) | **FHIR API clients: OAuth 2.0 client credentials** (spec §14.1) | Another system (HIS, payroll) reads the FHIR API without a person signed in. A System Admin registers it (Nursing Administration → API clients, `apiclients.manage`): a public `client_id` (`cl_…`) and a secret (`nwcs_…`, 256 bits) shown once and stored only as its SHA-256; scopes `system/Practitioner.read` and/or `system/PractitionerRole.read` (SMART v1 style, read only). `POST /api/v1/fhir/token` (RFC 6749 §4.4; credentials in HTTP Basic or the form body) returns a 15-minute HS256 Bearer token signed with a key derived from `JWT_SECRET` for this purpose only (HKDF), so a client token never passes as a user token nor the reverse. Every FHIR request re-reads the client: a new secret (`secret_version`) or a revocation ends its tokens at once. A client reads **system-wide** within its scopes, only `GET /api/v1/fhir/*`; any other path answers 401. Failed attempts throttled per address and per client id (D-46) and audited `API_CLIENT_AUTH_FAILED`; create, new secret and revoke audited HIGH. Request log: role `API_CLIENT`, the client id in place of the session. **Not chosen:** mutual TLS or private-key JWT client authentication (needs the other system's PKI; can be added beside the secret), per-client unit scoping (the integrations need the whole workforce) |
| D-64 (2026-09-25) | **SCFHS licence verification behind a gateway, simulated until U3** (spec §5.4) | `lib/scfhs.ts`: `ScfhsGateway.lookup(reg)` with `MockScfhsGateway` (answers from `mock_scfhs_registry`, kept by a System Admin in Nursing Administration → SCFHS registry; ERROR simulates an outage, an unlisted number is NOT_FOUND) and a `LiveScfhsGateway` stub for the SCFHS API (`SCFHS_DRIVER`, `SCFHS_API_URL`, `SCFHS_API_KEY`). Per credential type: `scfhs_enabled` (needs a `SCFHS_REG` field) and `scfhs_auto_suspend` (needs the checks; four-eyes like every catalog change). `modules/credentials/scfhs.ts` checks on submission (`POST /credentials`, self-service too), on demand (`POST /credentials/:id/scfhs-check`, scoped HR) and nightly (`scfhs-sync` 05:00, credentials PendingVerification/Valid/ExpiringSoon; stops after 5 failures in a row). Each check → `scfhs_verification_log` (number as its last four characters, response as SHA-256, discrepancies, action). Outcomes: agreement → nothing; a different expiry date, EXPIRED on a live credential, or NOT_FOUND → HIGH notice to scoped HR, once per answer — **SCFHS never overwrites the record**; SUSPENDED/REVOKED → with auto-suspend the credential is **Suspended** at once (grace closed, eligibility re-evaluated, audited HIGH `CREDENTIAL_SCFHS_SUSPENDED`), otherwise HR told. **Deviation from the spec example:** an SCFHS REVOKED suspends rather than revokes — revocation is final in V04 and stays an HR decision; the spec's `STALE 48 h` label is replaced by the job-freshness check (26 h) in System health, plus `SCFHS_UNREACHABLE` / `SCFHS_ERRORS` |
| D-65 (2026-09-25) | **Badge-event ingest, and a badge simulator until the badge system is connected** (spec §14.2; B-15) | `POST /api/v1/attendance/events` (`modules/attendance/ingest.ts`): the badge system signs in as an API client (D-63) holding the new **`attendance.ingest`** scope — a person never can (permission `attendance.ingest` has no role). Our format until the PACS contract: `{events: [{jobNumber, type, at, deviceId?, location?}]}`, 1–1000; `at` must carry its offset. Each event judged alone (unknown job number, deleted employee, > 5 min in the future, > 7 days old, invalid → rejected with its index); idempotent on (employee, instant, type) so a batch can be resent; source `client:<client id>`. Dev Console **badge simulator** (`devconsole.badge.*`, System Admin): one swipe, or clock-ins 0–20 min before the start for a unit's published shift with `leaveOut` nurses left out at random (→ MISSING and the coverage alert); source `simulator`, audited HIGH, clearable; **off in production** unless `BADGE_SIMULATOR=on` (fabricated attendance must not reach payroll). The client-token path in `authenticate` now also admits `POST /attendance/events`. **Not chosen:** badge-number mapping (the job number is the key; a badge→employee table can come with the PACS contract), a push stream (batches are what most PACS exports offer) |
| D-66 (2026-09-26) | **Telegram replaces SMS; messages carry no personal data** (amends D-48, D-59; spec §3.6, §8.3.6) | Owner decision: a live Saudi SMS gateway needs a CST-registered Sender ID (and so the hospital's Commercial Registration); Telegram needs only a bot. `lib/telegram.ts`: `TelegramGateway.send(chatId, text, {markdown?})` → `{accepted, messageId}`, never throws; `MockTelegramGateway` (saves to `mock_telegram_outbox`, Dev Console → Telegram inbox, 30-day purge — works offline) and `BotApiTelegramGateway` (`sendMessage`, 10 s timeout, token never logged); `NOTIFICATION_DRIVER` = `mock` (default) \| `telegram`, with `TELEGRAM_BOT_TOKEN` and `TELEGRAM_BOT_USERNAME`. The SMS gateway, its Unifonic stub, `mock_sms_outbox` (intercepted demonstration texts only) and `BREAK_GLASS_ALERT_PHONES` are removed. Break-glass messages `BREAK_GLASS_ALERT_TELEGRAM_CHAT_IDS` (audited `BREAK_GLASS_TELEGRAM_SENT` / `_FAILED`). Owner decision on content: Telegram's servers are outside the Kingdom, so a message holds a generic prompt, times and a link — never a name, unit, shift, credential or the client address. Owner decision on scope: notify-only. Accounts connect with a single-use deep link `https://t.me/<bot>?start=<token>` (own account, or HR for an account in scope; 15 min; SHA-256 stored; `/start` links, `/stop` unlinks; one chat per account; updates by polling from the jobs process or a secret-checked webhook). The Telegram dispatcher announces pending notifications — one message per person per run with only a count, urgency and a link; 5 attempts; earlier notifications never sent; shift Accept / Decline is **not built** — what a decline does to a published roster is REQUIREMENT NOT ESTABLISHED |
| D-67 (2026-09-26) | **Changing an account's sign-in e-mail: never instant** (spec §3.3) | Owner decisions: the account holder (My account → Security) **and** HR / an elevated System Admin for an active account in scope (`accounts.write`; never break-glass, never one's own) may request it; it applies only when the link sent to the **new** address is opened. `modules/users/email-change.ts`, table `email_changes` (SELF \| ASSISTED; one open request per account by partial unique index; only the token's SHA-256; 30 minutes; single use; a new request revokes the open one). SELF needs the current password (wrong ones count on the sign-in limit and are audited HIGH). The old address is warned at request time and told after the change (new address masked `a•••@domain`). Confirming (`POST /auth/email-change/confirm`, public, Origin check, per-client throttle) re-checks the address is free, switches it, signs out every session, audits `EMAIL_CHANGED` HIGH. E-mail must be on (`EMAIL_OFF` otherwise). The access token carries only the account id, so no token needs re-issuing |
| D-68 (2026-09-26) | **Development-only two-factor exemption for one account** (spec §3.5; complements the role switch `npm run mfa`) | Owner decision: `npm run mfa -w backend -- off <email\|id>` removes the account's authenticator and recovery codes and sets `users.mfa_exempt`, so it signs in with the password only whatever its role requires; `on <email\|id>` clears it (a required role sets up a new authenticator at the next sign-in). Read at each sign-in (no restart); audited HIGH `MFA_DEV_EXEMPTED` / `MFA_DEV_EXEMPTION_CLEARED`. The command refuses `NODE_ENV=production`, and the API ignores the flag there (`mfa.ts`), so a copied development database cannot weaken production. Break-glass is never asked anyway |
...
| Login attempt limits | Per account and per client, fixed window — in memory at first; **in PostgreSQL since D-46** (`login_throttle`) | Spec §3.3 requires limits but gives no numbers. Defaults 5/15 min per account, 20/15 min per client; configurable. **Confirmed by the owner (D-23)** |
...
| Break-glass (D-9) | Siren on successful sign-in of the flagged account: irrevocable event, HIGH audit, CRITICAL in-app notification to System Admins; root access without PAM/four-eyes; session and access end at 4 h. Failed attempts on it are audited HIGH. E-mail to the CEO and IT Director (D-47) and a text to them (D-59) | The text is simulated (mock SMS gateway) until the CST Sender ID exists (D-59) |
...
| Email | Notifications were stored with `emailStatus = SKIPPED` until SMTP was decided; since D-47 they are an outbox sent through the hospital relay. Unregistered employees (no account) still receive nothing | Built (D-47) |
