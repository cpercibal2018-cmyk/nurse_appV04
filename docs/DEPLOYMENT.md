...
| **No SMS; no password reset** | E-mail (§2.2) and registration by invitation are built; both need the relay and the private link from Google Cloud. SMS (the break-glass alert by text, D-48) and password reset are not built yet | The hospital SMS gateway's API details (D-48); an identity decision for password reset |
