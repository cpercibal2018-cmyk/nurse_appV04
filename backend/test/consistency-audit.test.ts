...
describeDb('consistency auditor (spec §10.8)', () => {
  ...
  it('GET /system/health/business reports drift, job freshness and e-mail delivery to System Admins only', async () => {
    ...
    expect(res.body.jobs.map((j: { name: string }) => j.name)).toEqual(['daily-transition', 'expiry-scan', 'attendance-alerts', 'consistency-audit', 'request-log-purge', 'mock-telegram-purge', 'scfhs-sync', 'vault-reconcile']);
