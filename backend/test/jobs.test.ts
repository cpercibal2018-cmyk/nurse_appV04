...
describeDb('jobs, notifications, audit and sessions', () => {
  ...
  describe('audit reading (D-20)', () => {
    it('is System Admin only, filters, and verifies the chain', async () => {
      ...
      expect(jobs.body.items.map((j: { name: string }) => j.name)).toEqual(['daily-transition', 'expiry-scan', 'attendance-alerts', 'consistency-audit', 'request-log-purge', 'mock-sms-purge', 'scfhs-sync', 'vault-reconcile']);
