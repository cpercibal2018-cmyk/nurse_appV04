...
// The 15-minute alert worker is commit 9; this is the on-demand view.
  ...
  return {
    ...
    async gaps(auth: AuthContext, q: z.infer<typeof GapsQuery>, now = new Date()) {
      ...
      for (const a of shifts) {
        const { start, end } = shiftWindow(dbDate(a.shiftDate), a.shiftType as ShiftType);
        ...
        let status: 'UPCOMING' | 'PENDING' | 'MISSING' | 'PRESENT' | 'INELIGIBLE_ON_DUTY';
        let reasons: string[] = [];
        if (clockIn) {
          status = 'PRESENT';
          if (date === today) {
            const r = evaluate(await loadFacts(db, a.employeeId, now), { date, today, now });
        ...
        items.push({
          assignmentId: a.id, employeeId: a.employeeId, jobNumber: a.employee.jobNumber, fullName: a.employee.fullName,
          shiftType: a.shiftType, shiftStart: start, shiftEnd: end, clockInAt: clockIn?.eventTimestamp ?? null, status, reasons,
        });
      }
